...
function buildScatter(parent: THREE.Group, field: Field, ground: Ground, rng: () => number): void {
  ...
  const species: Species[] = [
    { name: "creosote", geo: creosote(rng, 9, 0.28, 1.0, true), count: 450, weight: (_o, d, e) => 0.25 + 0.6 * d + 0.5 * e + 0.3 * o, scale: () => { const h = 0.8 + rng() * 1.1; return [h * (0.85 + rng() * 0.3), h, h * (0.85 + rng() * 0.3)]; }, tint: (c) => c.setRGB(0.26 + rng() * 0.1, 0.3 + rng() * 0.08, 0.17 + rng() * 0.06), shadow: 0.9, cast: true },
    ...
    { name: "rock-l", geo: rock(rng, 0.42), count: 320, weight: (_o, d) => 0.4 + 0.4 * (1 - d) + 0.3 * o, scale: () => { const s = 0.6 + rng() * 1.3; return [s * (0.8 + rng() * 0.5), s * (0.55 + rng() * 0.5), s * (0.8 + rng() * 0.5)]; }, tint: (c) => { const k = 0.24 + rng() * 0.18; c.setRGB(k * 1.06, k * 0.98, k * 0.9); }, shadow: 0.5, cast: true, minR: 12 },
  ];
  ...
  for (const sp of species) {
    ...
    const mesh = new THREE.InstancedMesh(sp.geo, mat, sp.count);
