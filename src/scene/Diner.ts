...
export class Diner {
  ...
  async build(hooks: BuildHooks): Promise<void> {
    ...
    await hooks.stage("Compiling shaders", 1);
    {
      ...
      // The sun never moves and nothing sunlit moves (the fan hangs above the window
      // head, out of the beam), so the shadow map is rendered once, here, and every
      // probe face and every frame after reuses it. See Diner.invalidateShadows().
  ...
  /** Call when the sun, the blinds, the door or anything else sunlit changes. */
