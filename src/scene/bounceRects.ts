 ...
 */
/**
 * Per-quad constants packed for the `uBounce` uniform array (8 vec4 per quad, see the GLSL):
 * [C.xyz far] [N.xyz 0] [L.xyz 0] [Lpt.xyz 0] [V0] [V1] [V2] [V3]. Lives in a uniform, not in
 * GLSL `const` arrays: ANGLE lowers a dynamically indexed const array to per-invocation
 * indexable temps (the whole table copied into registers for every fragment), which cost
 * 90+ ms of scene pass at 1080p. A cbuffer indexed in a loop costs nothing of the kind.
 */
...
export function bounceRectsGlsl(quads: BounceQuad[], zInside = ROOM.zFront): string {
  ...
  // perf-boot: ONE loop body over the quad table instead of ~43 unrolled copies of it. Rev 7's
  // 9° sun doubled the quad count (three height bands per strip, floor + partition + end-wall
  // landings) and the unrolled form made every physical program's HLSL compile ~10× slower on
  // ANGLE/D3D11 (173 programs: 11 s → 112 s of boot). perf-frame: the table is the `uBounce`
  // uniform array (bounceRectsUniform), bound by the PCSS wrapper in Lighting.ts — see there
  // for why not GLSL const arrays. The math per quad is unchanged: the same early-outs, point
  // form and Eberly acos.
    ...
    for ( int i = 0; i < BOUNCE_N; i ++ ) {
      ...
      if ( ce > 0.01 ) {
        if ( r2 > cf.w ) {
          ...
          vec3 d0 = uBounce[ b + 4 ].xyz - p, d1 = uBounce[ b + 5 ].xyz - p, d2 = uBounce[ b + 6 ].xyz - p, d3 = uBounce[ b + 7 ].xyz - p;
          if ( max( max( dot( n, d0 ), dot( n, d1 ) ), max( dot( n, d2 ), dot( n, d3 ) ) ) > 0.0 ) {
            ...
            acc += uBounce[ b + 2 ].xyz * max( -ff, 0.0 );
          }
        }
      }
    }`;
