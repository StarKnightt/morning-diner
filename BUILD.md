...
- Draw calls sit around 100–240 per frame (the transmission pass for glass
  ...
  shadow map is rendered once at boot (`shadowMap.autoUpdate = false`; call
  `diner.invalidateShadows()` when the sun, blinds or door change) — that pass
  was ~110 draw calls and ~6 ms of GPU time per frame.
...
```
  ...
  1.6 s  stand-in + compile   a blank 512 px cubemap PMREM'd to the probes' size stands in as
                              scene.environment while ALL 94 programs are issued at once:
  ...
  ~9.2 s probes               room / prop / lot CubeCamera captures, PMREM'd, assigned;
                              the shadow map renders once inside the first probe face
...
`?nofill` renders the sun alone (diagnostic).
